...
class generator_Middle(tf.keras.Model):
  def __init__(self, filters, strides, padding):
      ...
      self.conv = layers.Conv2DTranspose(filters, kernel_size=3, strides=strides, padding=padding, use_bias=False, kernel_initializer=RandomNormal(stddev=0.02))
      self.bn = layers.BatchNormalization(momentum=0.9)
      self.relu = tf.keras.layers.ReLU()
  def call(self, x):
      x = self.conv(x)
...
class generator_Output(tf.keras.Model):
  def __init__(self, image_depth, strides, padding):
    ...
    self.conv = layers.Conv2DTranspose(image_depth, kernel_size=3, strides=strides, padding=padding, use_bias=False, kernel_initializer=RandomNormal(stddev=0.02))
...
class discriminator_Input(tf.keras.Model):
  def __init__(self, filters, strides):
    ...
    self.conv = keras.layers.Conv2D(filters, kernel_size=3, strides=strides, padding="same", kernel_initializer=RandomNormal(stddev=0.02))
...
class discriminator_Middle(tf.keras.Model):
  def __init__(self, filters, strides, padding):
      ...
      self.conv = tf.keras.layers.Conv2D(filters, kernel_size=3, strides=strides, padding=padding, kernel_initializer=RandomNormal(stddev=0.02))
...
class discriminator_Output_channel(tf.keras.Model):
  def __init__(self, filters, strides, with_activation):
      ...
      self.conv = tf.keras.layers.Conv2D(filters, kernel_size=3, strides=strides, padding="same", kernel_initializer=RandomNormal(stddev=0.02))
